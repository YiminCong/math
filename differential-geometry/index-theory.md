...
2. **Solvability.** For which right-hand sides $f$ is $Du = f$ solvable? The obstructions live in the **cokernel** $\mathrm{coker} D = (\text{target space})/(\mathrm{im} D)$, where $\mathrm{im} D = \{Du\}$ is the image. Its dimension $\dim\mathrm{coker} D$ counts the constraints $f$ must satisfy.
...
turns out to be rigid. This difference is the **analytic index**. The miracle, due to Michael Atiyah and Isadore Singer in 1963, is that for an *elliptic* operator (a class we make precise in s2) on a compact manifold, $\mathrm{ind} D$ equals a number you can read off the topology of the manifold and the bundles involved — the **topological index** — with no reference to the operator's detailed coefficients at all.

#### Why this is surprising and useful

Consider an analogy with finite-dimensional linear algebra. If $A: \mathbb{R}^n \to \mathbb{R}^m$ is a matrix, the rank–nullity theorem gives $\dim\ker A = n - \mathrm{rank} A$ and $\dim\mathrm{coker} A = m - \mathrm{rank} A$, so
...
> **Intuition.** Think of the index as a "topologically protected" count, like the number of times a closed loop winds around a hole: you can wiggle the loop, but the winding number jumps only if you tear it. Wiggling $D$ moves $\ker$ and $\mathrm{coker}$, but solutions can only appear or vanish *in matched pairs*, leaving the difference fixed.
...
> - the image $\mathrm{im} T = \{Tx : x \in H_1\}$ is closed in $H_2$;
...
Here "bounded" means there is a constant $C$ with $\|Tx\| \le C\|x\|$ for all $x$, equivalently $T$ is continuous. The quotient $H_2/\mathrm{im} T$ is the space of equivalence classes $[y]$ where $y \sim y'$ iff $y - y' \in \mathrm{im} T$; its dimension measures how far $T$ is from being onto.

A clean reformulation uses the **adjoint** $T^*: H_2 \to H_1$, defined by $\langle Tx, y\rangle = \langle x, T^* y\rangle$ for all $x,y$. When $\mathrm{im} T$ is closed, the closed-range theorem (Functional Analysis guide) gives the orthogonal decomposition $H_2 = \mathrm{im} T \oplus (\mathrm{im} T)^\perp$ and the identity $(\mathrm{im} T)^\perp = \ker T^*$. Hence $\mathrm{coker} T \cong \ker T^*$ as vector spaces, and
...
> - $\mathrm{im} S = \{y : y_0 = 0\}$, which is closed, and $\mathrm{coker} S$ is one-dimensional (spanned by the class of $(1,0,0,\dots)$). So $\dim\mathrm{coker} S = 1$.
> - Therefore $\mathrm{ind} S = 0 - 1 = -1$.
>
> The left shift $S^*(x_0, x_1, \dots) = (x_1, x_2, \dots)$ is the adjoint, with $\ker S^* = \mathrm{span}\{(1,0,0,\dots)\}$, $\dim = 1$, confirming $\mathrm{ind} S = \dim\ker S - \dim\ker S^* = 0 - 1 = -1$. The power $S^k$ has index $-k$: indices range over all of $\mathbb Z$, so the integer is genuinely informative.

#### The stability theorem and its proof

> **Theorem (stability of the index).** Let $T: H_1 \to H_2$ be Fredholm.
> (a) If $K: H_1 \to H_2$ is **compact** (maps bounded sets to sets with compact closure), then $T + K$ is Fredholm and $\mathrm{ind}(T+K) = \mathrm{ind} T$.
> (b) There is $\varepsilon > 0$ such that every bounded $B$ with $\|B\| < \varepsilon$ has $T+B$ Fredholm and $\mathrm{ind}(T+B) = \mathrm{ind} T$.
...
**Proof of Atkinson (⇒).**
1. Suppose $T$ is Fredholm. By definition $\ker T$ is finite-dimensional and $\mathrm{im} T$ is closed with finite-dimensional complement. Restrict $T$ to $(\ker T)^\perp$; the restriction $T_0: (\ker T)^\perp \to \mathrm{im} T$ is a bounded bijection between Hilbert spaces (injective because we quotiented the kernel, surjective onto the image). *Reason:* removing the kernel makes $T$ injective; its image is unchanged.
2. By the bounded inverse theorem (a complete-space consequence of the open mapping theorem, Functional Analysis guide), $T_0^{-1}: \mathrm{im} T \to (\ker T)^\perp$ is bounded.
3. Define $P: H_2 \to H_1$ by $P = T_0^{-1} \circ \Pi$, where $\Pi: H_2 \to \mathrm{im} T$ is the orthogonal projection. Then $P$ is bounded (composition of bounded maps).
4. Compute $TP - I$. On $\mathrm{im} T$, $TP = T T_0^{-1} = I$; on $(\mathrm{im} T)^\perp$, $\Pi = 0$ so $TP = 0$. Thus $TP - I = -(I - \Pi)$, the negative of the projection onto $(\mathrm{im} T)^\perp$, which is finite-rank (since $\mathrm{coker} T$ is finite-dimensional) and hence compact. *Reason:* finite-rank operators are compact.
...
2. To see the index does not change, consider the path $T_t = T + tK$, $t \in [0,1]$. Each $T_t$ is Fredholm by step 1. We show $t \mapsto \mathrm{ind} T_t$ is locally constant; being integer-valued and (we show next) continuous, it is constant on $[0,1]$.

**Proof of stability (b), local constancy.** This is the key lemma; (a)'s constancy then follows. We use two standard inputs about the index, both proved from the definitions in the Functional Analysis guide: it is **additive under composition**, $\mathrm{ind}(AB)=\mathrm{ind} A+\mathrm{ind} B$ for Fredholm $A,B$, and it **vanishes on invertibles and on $I+(\text{compact})$** (the latter is the Riesz theory: $I+\text{compact}$ is Fredholm of index $0$).
1. Fix Fredholm $T$ and a parametrix $P$ (Atkinson), so $PT=I+K_1$ with $K_1$ compact. Additivity applied to $PT$ gives $\mathrm{ind} P+\mathrm{ind} T=\mathrm{ind}(I+K_1)=0$, hence $\mathrm{ind} P=-\mathrm{ind} T$. *Reason:* additivity and the index of $I+\text{compact}$.
2. Take any bounded $B$ with $\|B\|<\varepsilon:=1/\|P\|$ (if $P=0$ then $T$ is invertible-modulo-nothing and the claim is trivial). Then $\|PB\|\le\|P\|\,\|B\|<1$, so $I+PB$ is invertible by the Neumann series $\sum(-PB)^k$. *Reason:* a norm-$<1$ perturbation of the identity is invertible.
3. Compute $P(T+B)=PT+PB=I+K_1+PB=(I+PB)+K_1$. Factor out the invertible part:
$$
P(T+B)=(I+PB)\big(I+(I+PB)^{-1}K_1\big).
$$
Here $(I+PB)^{-1}K_1$ is compact (bounded $\circ$ compact), so the right factor is $I+(\text{compact})$, of index $0$, and the left factor is invertible, of index $0$. By additivity $\mathrm{ind}\big(P(T+B)\big)=0$. In particular $T+B$ is Fredholm (a parametrix for it is $(\text{right factor})^{-1}\!\!\mod\text{compact}\cdot(I+PB)^{-1}P$). *Reason:* additivity and the indices of invertibles and $I+\text{compact}$.
4. Apply additivity once more to the product $P(T+B)$: $\mathrm{ind} P+\mathrm{ind}(T+B)=\mathrm{ind}\big(P(T+B)\big)=0$, so $\mathrm{ind}(T+B)=-\mathrm{ind} P=\mathrm{ind} T$ by step 1. This holds for every $\|B\|<\varepsilon$, which is exactly local constancy. $\qquad\blacksquare$

For (a), apply (b) along the path $T_t = T + tK$: cover $[0,1]$ by finitely many intervals on each of which the index is constant, so $\mathrm{ind} T_0 = \mathrm{ind} T_1$, i.e. $\mathrm{ind}(T) = \mathrm{ind}(T+K)$.

> **Why this matters.** Stability is *why* the index is topological. Two elliptic operators that can be deformed into each other through elliptic operators (or differ by lower-order, hence "compact-like", terms) have the same index. So the index depends only on a discrete deformation class — exactly the kind of data topology measures. Pitfall: the *individual* dimensions $\dim\ker T$ and $\dim\mathrm{coker} T$ are **not** stable; only the difference is. Never expect to compute $\dim\ker$ from topology.
...
is the **de Rham complex**. Because $d^2 = 0$ we have $\mathrm{im}(d:\Omega^{k-1}\to\Omega^k) \subseteq \ker(d:\Omega^k\to\Omega^{k+1})$, and the **de Rham cohomology** is the quotient
...
> **Theorem.** $\displaystyle \mathrm{ind}(d+d^*: \Omega^{\mathrm{ev}}\to\Omega^{\mathrm{odd}}) = \chi(M)$, the **Euler characteristic** $\chi(M) = \sum_{k=0}^n (-1)^k b_k$.

**Proof.**
1. The adjoint of $D = d+d^*: \Omega^{\mathrm{ev}}\to\Omega^{\mathrm{odd}}$ is $D^* = d+d^*: \Omega^{\mathrm{odd}}\to\Omega^{\mathrm{ev}}$ (since $(d)^* = d^*$ and $(d^*)^* = d$). So $\mathrm{ind} D = \dim\ker(D|_{\mathrm{ev}}) - \dim\ker(D|_{\mathrm{odd}})$.
...
> (b) **Hodge decomposition:** $\Omega^k = \mathcal H^k \oplus \mathrm{im} d \oplus \mathrm{im} d^*$, an orthogonal direct sum.
...
> **Analytic input.** The Hodge Laplacian $\Delta = dd^* + d^*d$ on $\Omega^k$ is elliptic (its symbol is $|\xi|^2\,\mathrm{id}$, computed exactly as in s4 step 3, squared), self-adjoint, and non-negative. By s3 it is Fredholm; being self-adjoint, $\mathrm{coker}\Delta \cong \ker\Delta = \mathcal H^k$, so $\mathrm{im}\Delta = (\ker\Delta)^\perp$ is closed and $\Omega^k = \mathcal H^k \oplus \mathrm{im}\Delta$ orthogonally, with $\mathcal H^k$ finite-dimensional.

Everything else is linear algebra on this decomposition.

#### Proof of (b) and (c) from the input

1. **Decompose the image of $\Delta$.** From the input, $\Omega^k = \mathcal H^k \oplus \mathrm{im}\Delta$. Now $\mathrm{im}\Delta = \mathrm{im}(dd^* + d^*d) \subseteq \mathrm{im} d + \mathrm{im} d^*$. Conversely both $\mathrm{im} d$ and $\mathrm{im} d^*$ are orthogonal to $\mathcal H^k$: if $\omega$ harmonic then $\langle\omega, d\alpha\rangle = \langle d^*\omega,\alpha\rangle = 0$ (as $d^*\omega=0$) and similarly $\langle\omega, d^*\beta\rangle = \langle d\omega,\beta\rangle = 0$. *Reason:* harmonic forms are $d$- and $d^*$-closed (s4 step 2).
2. **$\mathrm{im} d \perp \mathrm{im} d^*$.** $\langle d\alpha, d^*\beta\rangle = \langle d^2\alpha,\beta\rangle = 0$ since $d^2 = 0$. *Reason:* the de Rham complex is a complex.
3. Combining 1–2 with the input gives the orthogonal splitting $\Omega^k = \mathcal H^k \oplus \mathrm{im} d \oplus \mathrm{im} d^*$, proving (b).
...
For a complex vector bundle $E$ with connection of curvature $\Omega$ (an $\mathrm{End}(E)$-valued $2$-form), the **total Chern class** is $c(E) = \det\!\big(I + \tfrac{i}{2\pi}\Omega\big) = 1 + c_1 + c_2 + \cdots$, with $c_k$ a closed $2k$-form whose cohomology class is independent of the connection. Formally factor $c(E) = \prod_j (1 + x_j)$; the $x_j$ are the **Chern roots** (formal degree-$2$ classes; symmetric functions of them are genuine classes).

#### The Chern character

> **Definition — Chern character.** $\displaystyle \mathrm{ch}(E) = \sum_j e^{x_j} = \mathrm{rank}(E) + c_1 + \tfrac12(c_1^2 - 2c_2) + \cdots = \mathrm{tr}\,\exp\!\Big(\tfrac{i}{2\pi}\Omega\Big).$

Its defining virtue is additivity and multiplicativity:

> **Proposition.** $\mathrm{ch}(E\oplus F) = \mathrm{ch}(E) + \mathrm{ch}(F)$ and $\mathrm{ch}(E\otimes F) = \mathrm{ch}(E)\,\mathrm{ch}(F)$.

**Proof.** For a direct sum the Chern roots are the union $\{x_j\}\cup\{y_k\}$, so $\sum e^{x_j} + \sum e^{y_k}$ adds. For a tensor product the roots are the sums $x_j + y_k$, so $\mathrm{ch}(E\otimes F) = \sum_{j,k} e^{x_j+y_k} = (\sum_j e^{x_j})(\sum_k e^{y_k}) = \mathrm{ch}(E)\mathrm{ch}(F)$, using $e^{a+b}=e^a e^b$. *Reason:* the exponential turns sums of roots into products. $\qquad\blacksquare$

This is precisely why the Chern character is the natural object on the topological side: the index is additive under direct sums of operators and multiplicative under products of manifolds, and $\mathrm{ch}$ is the universal class with those properties.
...
- **The left side is analysis.** $\dim\ker D$ and $\dim\mathrm{coker} D$ require solving a PDE on $M$ — finding all solutions of $Du = 0$ and all obstructions. These depend in a complicated way on the metric, the connection, the precise coefficients. Each is genuinely hard and individually metric-dependent.
- **The right side is topology.** $\mathrm{ch}, \mathrm{Td}, \hat A$ are polynomials in curvature whose *integrals* are independent of all choices (Chern–Weil: the integral of a characteristic class is a homotopy/cobordism invariant). You can compute the right side from the manifold's topology and the bundles' topology alone, often by hand.
...
By Chern–Weil, the Euler class is represented in even dimension $n = 2m$ by the **Pfaffian** of the curvature: $e(TM) = \frac{1}{(2\pi)^m m!}\mathrm{Pf}(\Omega)$. In two dimensions this is $\frac{1}{2\pi}K\,dA$ with $K$ the Gauss curvature, giving the classical **Gauss–Bonnet theorem**

$$
\chi(M^2) = \frac{1}{2\pi}\int_M K\, dA.
$$

> **Worked example.** For the unit $S^2$, $K = 1$ and $\mathrm{Area} = 4\pi$, so $\frac{1}{2\pi}\int K\,dA = \frac{4\pi}{2\pi} = 2 = \chi(S^2)$. Three computations — counting harmonic forms (s4), the curvature integral here, and the topology $1-0+1$ — all give $2$.

#### The Hirzebruch signature theorem (operator: the signature operator)

On an oriented manifold of dimension $n = 4k$, the **Hodge star** $\star$ gives an involution $\tau$ on middle-dimensional forms, splitting $\Omega^{\mathrm{ev}} = \Omega^+\oplus\Omega^-$ into $\pm 1$ eigenspaces. The **signature operator** $D_{\mathrm{sig}} = d+d^*:\Omega^+\to\Omega^-$ has analytic index equal to the **signature** $\mathrm{sign}(M)$ — the signature of the intersection form on $H^{2k}(M)$ (number of positive minus negative eigenvalues of the symmetric pairing $\alpha\wedge\beta$). The topological index is the **$L$-genus** (a Pontryagin polynomial):

$$
\mathrm{sign}(M) = \int_M L(M), \qquad L = 1 + \tfrac13 p_1 + \tfrac{1}{45}(7p_2 - p_1^2) + \cdots
$$

> **Worked example.** On $M^4$, $\mathrm{sign}(M) = \tfrac13\int_{M} p_1$. For the complex projective plane $\mathbb{CP}^2$, $\int p_1 = 3$ (since $p_1 = 3$ generator), giving $\mathrm{sign} = 1$ — correct, as $H^2(\mathbb{CP}^2)=\mathbb Z$ with positive self-intersection.
...
3. Only $\lambda = 0$ survives: $\dim V_0^+ - \dim V_0^- = \dim\ker D - \dim\ker D^* = \mathrm{ind} D$, with $e^{-t\cdot 0} = 1$. Hence the supertrace equals $\mathrm{ind} D$ for all $t$. $\qquad\blacksquare$
...
2. **Only the constant term can survive.** Integrating, $\mathrm{ind} D = \int_M \mathrm{Str} k_t(x,x)\,dV \sim (4\pi t)^{-n/2}\sum_j t^j\int_M a_j$. The left side is the constant $\mathrm{ind} D$, so all powers of $t$ except $t^0$ must vanish in the limit: the $t^{-n/2+j}$ terms with $j < n/2$ integrate to zero, and the $t\to0$ limit picks out $j = n/2$ (for even $n$), giving
$$
