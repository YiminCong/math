...
**Proof of stability (b), local constancy.** This is the key lemma; (a)'s constancy then follows. We use two standard inputs about the index, both proved from the definitions in the Functional Analysis guide: it is **additive under composition**, $\operatorname{ind}(AB)=\operatorname{ind} A+\operatorname{ind} B$ for Fredholm $A,B$, and it **vanishes on invertibles and on $I+(\text{compact})$** (the latter is the Riesz theory: $I+\text{compact}$ is Fredholm of index $0$).
1. Fix Fredholm $T$ and a parametrix $P$ (Atkinson), so $PT=I+K_1$ with $K_1$ compact. Additivity applied to $PT$ gives $\operatorname{ind} P+\operatorname{ind} T=\operatorname{ind}(I+K_1)=0$, hence $\operatorname{ind} P=-\operatorname{ind} T$. *Reason:* additivity and the index of $I+\text{compact}$.
...
$$
Here $(I+PB)^{-1}K_1$ is compact (bounded $\circ$ compact), so the right factor is $I+(\text{compact})$, of index $0$, and the left factor is invertible, of index $0$. By additivity $\operatorname{ind}\big(P(T+B)\big)=0$. In particular $T+B$ is Fredholm (a parametrix for it is $(\text{right factor})^{-1}\!\!\mod\text{compact}\cdot(I+PB)^{-1}P$). *Reason:* additivity and the indices of invertibles and $I+\text{compact}$.
4. Apply additivity once more to the product $P(T+B)$: $\operatorname{ind} P+\operatorname{ind}(T+B)=\operatorname{ind}\big(P(T+B)\big)=0$, so $\operatorname{ind}(T+B)=-\operatorname{ind} P=\operatorname{ind} T$ by step 1. This holds for every $\|B\|<\varepsilon$, which is exactly local constancy. $\qquad\blacksquare$
