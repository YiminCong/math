...
> 2. **Order $\hbar^1$.** The $k=1$ term is $\tfrac{i\hbar}{2}\sum_{ij}\pi^{ij}(\partial_i f)(\partial_j g)=\tfrac{i\hbar}{2}\,\pi(df,dg)$. So $B_1(f,g)=\tfrac{i}{2}\pi(df,dg)$. Its antisymmetric part is $B_1(f,g)-B_1(g,f)=\tfrac{i}{2}\pi(df,dg)-\tfrac{i}{2}\pi(dg,df)=i\,\pi(df,dg)$. With the standard convention $\{f,g\}=\tfrac{1}{i}[f,g]_\star+O(\hbar)$ this matches (S2) up to the universal factor $i$ that the $\tfrac{i\hbar}{2}$ convention introduces; in the real convention ($\tfrac{\hbar}{2}\overleftrightarrow{P}$) one gets exactly $\{f,g\}$.
...
4. So $B_1'(f,g)=B_1(f,g)+\big(T_1f\,g+f\,T_1g-T_1(fg)\big)$. The added term is *symmetric* in $f,g$ if $T_1$ is a derivation, and in general its antisymmetrization vanishes because $T_1f\,g+f\,T_1g-T_1(fg)$ is symmetric in $(f,g)$ whenever it is — indeed swapping $f\leftrightarrow g$ leaves it unchanged. Therefore $B_1'(f,g)-B_1'(g,f)=B_1(f,g)-B_1(g,f)=\{f,g\}$. $\qquad\blacksquare$
