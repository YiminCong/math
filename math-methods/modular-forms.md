...
The companion law $\theta(\tau+2)=\theta(\tau)$ holds because a shift $\tau\to\tau+1$ multiplies the $n$-th term by $e^{\pi i n^2}=\pm1$: explicitly $\theta(\tau+1)=\sum e^{\pi i n^2(\tau+1)}=\sum e^{\pi i n^2}q^{n^2/2}$, which is a *different* theta-constant (the "third" theta) rather than $\theta$ itself, so $\theta$ has period $2$, not $1$. Together with $S$ this generates the subgroup $\Gamma_\theta$.
