...
1. **There is a maximal-modulus eigenvalue.** Define $\|T\|=\sup_{\|x\|=1}|\langle x,Tx\rangle|$ (a theorem for self-adjoint $T$: the operator norm equals this numerical radius — proven via the polarization identity). Take $x_n$ with $\langle x_n,Tx_n\rangle\to\lambda_1$, $|\lambda_1|=\|T\|$. *(reduce to a variational problem)*
2. **Compactness produces an eigenvector.** Compute $\|Tx_n-\lambda_1 x_n\|^2=\|Tx_n\|^2-2\lambda_1\langle x_n,Tx_n\rangle+\lambda_1^2\le 2\lambda_1^2-2\lambda_1\langle x_n,Tx_n\rangle\to 0$. By compactness pass to a subsequence with $Tx_n\to y$; then $\lambda_1 x_n\to y$ too, so $x_n\to e_1:=y/\lambda_1$ (if $\lambda_1\ne0$), and $Te_1=\lambda_1 e_1$ with $\|e_1\|=1$. So $\lambda_1$ is a genuine eigenvalue. *(this is where compactness is essential — it converts a near-eigenvector into an exact one)*
