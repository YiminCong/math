...
The equivalence "positive $\iff a=b^*b$" is the algebraic backbone. One direction: if $a\ge0$ then by functional calculus $b=\sqrt a$ (using $f(z)=\sqrt z$ on $\sigma(a)\subseteq[0,\infty)$) is self-adjoint with $b^*b=b^2=a$. The converse — that $b^*b\ge0$ for every $b$ — is the nontrivial half and rests on the C\*-identity; it ensures the positive elements form a cone closed under addition.
...
3. At $z=t+i\beta$: $e^{i(t+i\beta)H}=e^{itH}e^{-\beta H}$, so $F_{a,b}(t+i\beta)=\frac{1}{Z}\mathrm{Tr}(e^{-\beta H}a\,e^{itH}e^{-\beta H}be^{\beta H}e^{-itH})$; using cyclicity of the trace to move $e^{-\beta H}$ around and $\sigma_t(b)=e^{itH}be^{-itH}$, this equals $\omega_\beta(\sigma_t(b)\,a)$. *(cyclicity of the matrix trace; $Z=\mathrm{Tr}\,e^{-\beta H}$.)*
