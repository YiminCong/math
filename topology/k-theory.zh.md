...
3. **拼接。** 用这样的邻域 $U_i$ 覆盖 $Y$，在其上 $E_H\cong p^*(E_H|_{U_i\times\{0\}})$。取从属于 $\{U_i\}$ 的单位分解 $\{\rho_i\}$（由仿紧性存在）。定义 $Y\times[0,1]$ 的一个自映射 $\Phi(y,t)=(y,\sum_i\rho_i(y)\,t_i)$ 方案——更干净地说：用每个 $U_i$ 上的同构，通过 $\{\rho_i\}$ 对平凡化标架作插值，从而构造一个整体丛同构；黏合的线性性与参数 $t$ 的凸性使该插值良定义且可逆。*（单位分解黏合局部同构；凸组合经同伦提升论证保持在 $GL$ 中）*
...
$$
我们已求得 $\tilde K^0(S^0)=0$（两个点：$K^0=\mathbb{Z}\oplus\mathbb{Z}$，约化部分是差，但 $S^0$ 的约化平凡……仔细地说：$\tilde K^0(S^0)=\mathbb{Z}$，来自两个分支减对角——见下文）以及 $\tilde K^0(S^2)=\mathbb{Z}$。完整模式 $\tilde K^0(S^n)=\mathbb{Z}$（$n$ 偶）、$0$（$n$ 奇）正是 Bott 周期性的内容，我们现在转向它。具体地它给出
$$
...
**演示 —— 降次技巧的具体推演。** 第 2 步的核心在于：一条单项式型的黏合环路可以降一次，代价是把丛扩大一个平凡直和项。具体地，对 $S^1$ 上的环路 $z\mapsto z\cdot\mathrm{id}_{\mathbb{C}^n}$：
