...
3. **Patching.** Cover $Y$ by such neighborhoods $U_i$ on which $E_H\cong p^*(E_H|_{U_i\times\{0\}})$. Choose a partition of unity $\{\rho_i\}$ subordinate to $\{U_i\}$ (exists by paracompactness). Define a self-map of $Y\times[0,1]$, $\Phi(y,t)=(y,\sum_i\rho_i(y)\,t_i)$ scheme — more cleanly: use the isomorphisms over each $U_i$ to build a global bundle isomorphism by interpolating the trivializing frames with $\{\rho_i\}$; linearity of the gluing and convexity of the parameter $t$ make the interpolation well-defined and invertible. *(partition of unity glues local isomorphisms; convex combination stays in $GL$ via a homotopy-lifting argument)*
...
$$
We already found $\tilde K^0(S^0)=0$ (two points: $K^0=\mathbb{Z}\oplus\mathbb{Z}$, reduced part the difference, but $S^0$ has trivial reduced... carefully: $\tilde K^0(S^0)=\mathbb{Z}$ from the two components minus diagonal — see below) and $\tilde K^0(S^2)=\mathbb{Z}$. The full pattern $\tilde K^0(S^n)=\mathbb{Z}$ for $n$ even and $0$ for $n$ odd is the content of Bott periodicity, to which we now turn. Concretely it gives
$$
...
**Demonstration — the degree-shift trick, worked.** The heart of step 2 is that a clutching loop of monomial type can be lowered in degree by one at the cost of enlarging the bundle by a trivial summand. Concretely, for the loop $z\mapsto z\cdot\mathrm{id}_{\mathbb{C}^n}$ on $S^1$:
