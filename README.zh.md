...
课程，每一科都从基础一路讲到研究生级别的严格理论。每份指南都提供 **Markdown**
版本（可直接在 GitHub 上阅读）以及单文件 **HTML** 版本（带
[KaTeX](https://katex.org/) 数学渲染的交互式、可搜索页面）。

## 开始阅读

每份指南有两种形式：

- **Markdown**（`.md`）—— 在 GitHub 上直接渲染，可从下方表格进入。
- **HTML**（`.html`）—— 单文件、自成体系，打开即为交互式、可搜索、带 KaTeX
  数学渲染的页面。

想要交互式版本？打开 **[`index.html`](index.html)** 即可获得带各科学习路径、主题
索引和全文搜索框的导航页（界面为英文，见 [本地查看](#本地查看)）。

> 英文版请见 **[README.md](README.md)**。术语表本身即为英↔中双语，两种语言通用。
...
直接双击即可打开文件；若想让页内链接与搜索像网站一样工作，可在仓库根目录启动一个
本地服务器：
...
字体与 KaTeX 数学字体从 CDN 加载，因此联网时显示效果最佳。
